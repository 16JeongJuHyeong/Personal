...
public class Target_Hostile_byClass : Target_byClass
{
    ...
    void Update()
    {
        if (Time_Left.game_time > 0)
        {
            ...
            {
                transform.position = transform.position + new Vector3(x_direction, y_direction, 0).normalized * Time.deltaTime/2 * (!Pause.IsPause ? 1 : 0) * TimeManager.time_flow;
                transform.Rotate(0,0,0.08f * (!Pause.IsPause ? 1 : 0) * TimeManager.time_flow);
